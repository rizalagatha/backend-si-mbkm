...
const createPendaftaranMbkm = async (req, res) => {
  const { id_pendaftaran_mbkm, NIM, tanggal, id_program_mbkm, NIP_dosbing, id_matkul_knvrs, status } = req.body;
  try {
    ...
    const matkulKnvrs = await MatkulKnvrs.findByPk(id_matkul_knvrs);

    if (!mahasiswa || !programMbkm || (NIP_dosbing && !dosbing && !matkulKnvrs)) {
      return res.status(400).json({
        message: 'Related Mahasiswa, Dosen Pembimbing, Program MBKM, or Matkul Konversi not found',
      });
    }    
...
const updatePendaftaranMbkm = async (req, res) => {
  ...
  try {
    const [updated] = await PendaftaranMbkm.update(
      { NIM, tanggal, id_program_mbkm, NIP_dosbing, id_matkul_knvrs, status },
